...
public interface ContaFinanceiraRepository extends JpaRepository<ContaFinanceira, Long> {
   ...
   * @return Valor total integralizado (soma dos valores positivos menos os
   *         reembolsos)
   */
  @Query("""
          SELECT COALESCE(SUM(
  ...
  Integer obterUltimoNroParcelaContratoOrigem(@Param("idContrato") Long idContrato,
      @Param("idOrigemConta") Integer idOrigemConta);
