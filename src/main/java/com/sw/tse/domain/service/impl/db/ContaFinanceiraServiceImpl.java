...
public class ContaFinanceiraServiceImpl implements ContaFinanceiraService {
    ...
    public List<ContaFinanceiraClienteDto> buscarContasClienteDto(LocalDate vencimentoInicial,
            LocalDate vencimentoFinal, String status) {
    ...
    public ContasPaginadasDto buscarContasClienteDtoComPaginacao(
        ...
        log.info("Processando busca para cliente ID: {}. Filtros: Venc {} a {}, Status {}, Empresa {}",
                idCliente, vencimentoInicial, vencimentoFinal, status, empresaId);
        ...
        List<ContaFinanceiraClienteDto> contasDto = contasFinanceiras.stream()
                .map(cf -> {
                    log.debug("Convertendo conta ID {} para DTO. Modo simulação: {}", cf.getId(), modoSimulacao);
                    return contaFinanceiraConverter.toDto(cf, modoSimulacao);
                })
                .collect(Collectors.toList());

        log.info("Busca concluída. {} contas convertidas para DTO.", contasDto.size());
    ...
    public byte[] gerarSegundaViaBoleto(Long idContaFinanceira) {
        ...
        // Validar se pertence ao cliente autenticado (idpessoa na conta OU
        // cessionário/co-cessionário do contrato)
        Long idCliente = JwtTokenUtil.getIdPessoaCliente();
        if (idCliente == null) {
            throw new TokenJwtInvalidoException("ID do cliente não está disponível no token de autenticação");
        }
        ...
        try {
            ...
            // Gerar segunda via - Feign automaticamente lança ApiTseException com mensagem
            // da API se houver erro
     ...
     * contas na lista.
     * 
     * Também pré-carrega os parâmetros financeiros em batch para evitar N+1
     * queries.
     * 
    ...
    private void inicializarRelacionamentosParaCalculo(List<ContaFinanceira> contasFinanceiras) {
        ...
        // Inclui empresa.pessoa e pessoa.enderecos para cálculo de juros (cidade/estado
        // para feriados)
        String jpql = """
                SELECT DISTINCT cf
        ...
        // Extrair IDs únicos de empresas que não têm carteiraBoleto (precisam de
        // ParametroFinanceiro)
        ...
        for (int i = 0; i < contasFinanceiras.size(); i++) {
            ...
            if (contaComRelacionamentos != null) {
                ...
                if ("VENCIDO".equals(status)) {
                    ...
                    log.info(
                            "Conta VENCIDA ID {} - CarteiraBoleto: {}, Empresa: {}, Juros: {}, Multa: {}, ValorAtualizado: {}",
                            contaComRelacionamentos.getId(),
                            infoCarteira,
                            infoEmpresa,
                            juros,
                    ...
                    if (juros.compareTo(BigDecimal.ZERO) == 0 && multa.compareTo(BigDecimal.ZERO) == 0) {
                        log.warn(
                                "ATENÇÃO: Conta VENCIDA ID {} não tem juros/multa calculados! CarteiraBoleto: {}, Empresa: {}",
                                contaComRelacionamentos.getId(),
                                infoCarteira,
                                infoEmpresa);
                    }
