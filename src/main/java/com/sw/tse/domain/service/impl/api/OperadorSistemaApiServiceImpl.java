...
public class OperadorSistemaApiServiceImpl implements OperadorSistemaService {
	...
	public OperadorSistemaListaApiResponse buscarPorId(Long idOperadorSistema) {
		...
		return this.listarTodos()
				...
				.findFirst().orElseThrow(() -> new OperadorSistemaNotFoundException(
	...
	public BuscaOperadorSistemPessoaResponse buscarPorIdPessoa(Long idPessoa) {
		...
		try {
			...
			throw new ApiTseException(String.format(
					"Erro ao obter operador sistema cliente pela api do TSE (HTTP %d). %s",
					e.status(),
					responseBody != null && !responseBody.isBlank() ? responseBody : "Resposta da API sem detalhes."));
		}
	}
		...
		try {
			List<BuscaOperadorSistemPessoaResponse> listaOperadorSistemPessoa = relatorioCustomizadoApiService
					.buscarRelatorioGenerico(idRelatorioOperadorSistema, filtros,
							BuscaOperadorSistemPessoaResponse.class);
			BuscaOperadorSistemPessoaResponse result = listaOperadorSistemPessoa.stream().findFirst()
					.orElse(new BuscaOperadorSistemPessoaResponse(idOperador, 0L, null, null, null, false));
			log.debug(
					"API TSE - Response sucesso ao obter operador sistema cliente por idOperador. idOperador={}, response={}",
					idOperador, result);
			return result;
		} catch (FeignException e) {
			String responseBody = e.contentUTF8();
			log.error(
					"API TSE - Response erro ao obter operador sistema cliente por idOperador. status={}, response={}",
					e.status(), responseBody != null ? responseBody : "(vazio)");
			throw new ApiTseException(String.format(
					"Erro ao obter operador sistema cliente pela api do TSE (HTTP %d). %s",
					e.status(),
		...
		try {
			List<BuscaOperadorSistemPessoaResponse> listaOperadorSistemPessoa = relatorioCustomizadoApiService
					.buscarRelatorioGenerico(idRelatorioOperadorSistema, filtros,
							BuscaOperadorSistemPessoaResponse.class);
			BuscaOperadorSistemPessoaResponse result = listaOperadorSistemPessoa.stream().findFirst()
					.orElse(new BuscaOperadorSistemPessoaResponse(0L, 0L, null, null, null, false));
			log.debug("API TSE - Response sucesso ao obter operador sistema cliente por login. login={}, response={}",
					login, result);
			return result;
		} catch (FeignException e) {
			String responseBody = e.contentUTF8();
	...
	public String resetarSenhaV2(ResetarSenhaV2RequestDto request) {
		log.info("Iniciando processo de reset de senha v2. Request: {}", request);
		...
		try {
			...
			// Chamada real ao endpoint do TSE
			String response = operadorSistemaApiClient.resetarSenhaV2(bearerToken, request);
			log.info("Resposta da API do TSE: {}", response);
			return response;

		} catch (feign.FeignException e) {
			log.error("Erro de API ao resetar senha v2: Status={}, Body={}", e.status(), e.contentUTF8());

			String erroMsg = e.contentUTF8();
			if (erroMsg.contains("Você não tem permissão para resetar a senha deste operador.")) {
				throw new ApiTseException("Você não tem permissão para resetar a senha deste operador.");
			}
