...
public class ProcessamentoPagamentoServiceImpl implements PagamentoCartaoService {
    ...
    public void processarPagamentoAprovado(ProcessarPagamentoAprovadoTseDto dto) {
        ...
        try {
            ...
            if (idUsuario != null && idUsuario > 0) {
                responsavel = operadorSistemaRepository.findById(idUsuario)
                        .orElseThrow(() -> new OperadorSistemaNotFoundException(
                                "Operador sistema não encontrado para ID: " + idUsuarioParaLog));
                ...
                if (resp != null && resp.idOperador() != null && resp.idOperador() > 0) {
                    responsavel = operadorSistemaRepository.findById(resp.idOperador())
                            .orElseThrow(() -> new OperadorSistemaNotFoundException(
                                    "Operador não encontrado para idPessoa: " + dto.getIdPessoaTse()));
                    log.info("Usuário responsável: ID {} (idPessoaTse {} - job)", responsavel.getId(),
                            dto.getIdPessoaTse());
                } else {
            ...
            if (!isPix) {
                ...
                transacaoRepository.save(transacao);
                log.info("TransacaoDebitoCredito atualizada - Status: {}, NSU: {}", transacao.getStatus(),
                        transacao.getNsu());
            } else {
            ...
            ContaFinanceira contaNova = criarContaConsolidada(contasOriginais, transacao, dto, responsavel,
            ...
            // 7. Criar Negociação
            Negociacao negociacao = criarNegociacao(contasOriginais.get(0).getEmpresa(), responsavel,
    ...
    private ContaFinanceira criarContaConsolidada(
        ...
        ContaFinanceira contaNova = ContaFinanceira.criarContaConsolidadaPagamentoPortal(
                ...
                dto.getDataPagamento() != null ? dto.getDataPagamento() : LocalDateTime.now(), // Data do pagamento
                                                                                               // (ajustada ou agora)
                dto.getDataAutorizacao() != null ? dto.getDataAutorizacao() : LocalDateTime.now(), // Data da
                                                                                                   // autorização/liquidação
                                                                                                   // (ou agora)
    ...
    private void configurarCamposAdicionaisContaNova(
        ...
        log.info("DEBUG - Configurando campos adicionais - Juros CALCULADOS: {}, Multa CALCULADA: {}", valorJuros,
                valorMulta);

        // Logar valores individuais das contas para debug
        contasOriginais
                .forEach(conta -> log.info("DEBUG - Conta ID: {}, Status: {}, Juros Calculado: {}, Multa Calculada: {}",
